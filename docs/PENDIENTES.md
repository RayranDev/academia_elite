...
>
> Última actualización: 2026-08-04 (noche, 3).
...
| **Aranceles: editar, evitar duplicados, describir "OTRO"** | Medio | Reportado en uso real (2026-08-04), verificado contra el código. Hoy `Arancel` solo tiene crear/desactivar, no editar (`arancel.repository.ts` no tiene `actualizarArancel`); no hay `@@unique` ni chequeo de duplicado por (categoría, concepto) — se puede cargar el mismo precio dos veces sin aviso; y no hay campo `descripcion` en el modelo, así que un concepto "OTRO" queda sin contexto para cuando alguien lo audite después. La página tampoco está en el `NAV` del sidebar (solo se llega vía el botón "Precios" de Membresías), así que no tiene una forma propia de "volver". |
| **Membresías: paginación, filtro por mes/jugador, export conectado al filtro** | Medio | Reportado en uso real, verificado. `listarMembresias`/`listarMembresiasEscuela` traen TODAS las filas sin `skip`/`take` — con uso real las cobranzas se acumulan sin límite. La UI solo filtra por `estado` (querystring); no hay filtro de período ni buscador de jugador en la lista (el combobox de jugador que existe es para el alta, no para filtrar). El botón "Descargar cobranza" ignora el filtro activo en pantalla: el backend (`export-membresias.service.ts`) sí soporta `?estado=`/`?periodo=`, pero la UI nunca los adjunta al link. |
| **Bloqueo por mora: acceso directo desde Membresías + acción masiva de morosos** | Grande | Reportado en uso real como "corrección grande", verificado. Hoy el bloqueo (`bloqueo.service.ts`) actúa sobre UN jugador por vez, solo alcanzable desde la ficha del jugador — nada conecta la lista de Membresías con bloquear a quien está en mora. Falta: (a) acceso directo desde la fila de una cuota vencida a la ficha/bloqueo de ese jugador, (b) una acción de "ver morosos" que liste a todos con cuota vencida y permita elegir a quién bloquear en un solo paso (no bloqueo automático ciego — el usuario elige). Requiere extender `bloquearAccesoJugador` para aceptar múltiples ids o iterar con la misma auditoría por jugador. |
| **Jugador bloqueado: acceso parcial a mensajes DT-jugador** | Medio | Pedido en uso real, contrasta con el comportamiento actual (verificado): hoy el bloqueo es TOTAL — `requireAuthContext`/`requirePanelUser` redirigen a `/bloqueado` antes de renderizar cualquier página de `/jugador`, incluidos los mensajes; la única acción disponible en `/bloqueado` es cerrar sesión. Lo pedido (dejar entrar con un aviso de "contactá a la escuela" y acceso solo a los mensajes directos DT-jugador) es un cambio de comportamiento en funciones de auth centrales (`requireAuthContext`/`requirePanelUser`, usadas en TODA la app) — amerita diseño propio antes de tocar código, no un parche puntual. |
| **Perfil del DT con estadísticas consolidadas** | Medio | Reportado en uso real, verificado: no existe ninguna pantalla de "mis estadísticas"/"resultados" para el DT (las rutas de `dt/` son agenda, plantilla, progreso —de los jugadores, no del DT—, logros, mensajes; nada agrega evaluaciones hechas ni resultados del equipo a nivel DT). Falta definir qué debería mostrar (¿evaluaciones hechas en el mes? ¿resultados del equipo? ¿ambos?) antes de construirlo. |
