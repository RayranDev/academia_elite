...
export function codigoInvitacion(
  ...
  return {
    ...
    html: layout(
      ...
      `<p>Usa este código para registrar a tu jugador en la categoría <strong>${escapar(categoria)}</strong>:</p>
       <p style="text-align:center;font-size:28px;font-weight:800;letter-spacing:6px;color:#4ade80;margin:16px 0;">${escapar(codigo)}</p>
       <p>Entra a la plataforma, elige "Registrarme con código" e ingresa ese código.</p>`,
    ),
